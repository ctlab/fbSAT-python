...
from . import ExtendedAutomatonTask, MinimalBasicAutomatonTask, Task
from ..efsm import EFSM
from ..printers import log_br, log_debug, log_error, log_info, log_success, log_warn
from ..utils import json_dump

__all__ = ['MinimalExtendedAutomatonTask']


class MinimalExtendedAutomatonTask(Task):

    def __init__(self, scenario_tree, *, C=None, K=None, P, N_init=None, use_bfs=True, is_distinct=False, is_forbid_or=False, solver_cmd, is_incremental=False, is_filesolver=False, outdir=None, path_output=None):
        ...
        self.subtask_config_extended = dict(**self.subtask_config_minbasic,
    ...
    def save_params(self):
        params = dict(C=self.C, K=self.K, N_init=self.N_init, use_bfs=self.use_bfs, is_distinct=self.is_distinct, is_forbid_or=self.is_forbid_or, solver_cmd=self.solver_cmd, is_incremental=self.is_incremental, is_filesolver=self.is_filesolver, outdir=str(self.path_output))
    ...
    def create_extended_subtask_call(self, C, K, P):
        ...
        config = dict(**self.subtask_config_extended, **call_params, path_output=path_call)
    ...
    def run(self, *, fast=False):
        ...
        log_debug(f'MinimalExtendedAutomatonTask: running...')
        time_start_run = time.time()
        best = None

        if self.C is None:
            log_debug('MinimalExtendedAutomatonTask: searching for minimal C...')
            task_call = self.create_basic_min_subtask_call()
            assignment = task_call()
            C = assignment.C
            log_debug(f'MinimalExtendedAutomatonTask: found minimal C={C}')
        else:
            C = self.C
            # log_debug(f'MinimalExtendedAutomatonTask: using specified C={C}')

        if self.K is None:
            K = C
            # log_debug(f'MinimalExtendedAutomatonTask: using K=C={K}')
        else:
            K = self.K
            # log_debug(f'MinimalExtendedAutomatonTask: using specified K={K}')

        task_call, task_finalize = self.create_extended_subtask_call(C, K, self.P)
        assignment = task_call(self.N_init)

        while assignment:
            log_debug('MinimalExtendedAutomatonTask: searching for minimal N...')
            best = assignment
            N = best.N - 1
            log_br()
        ...
        if fast:
            ...
            log_debug(f'MinimalExtendedAutomatonTask: done in {time_total_run:.2f} s')
            return best
        else:
            ...
            log_br()
            if automaton:
                log_success(f'Minimal extended automaton has {automaton.C} states, {automaton.T} transitions and {automaton.N} nodes')
            else:
    ...
    def build_efsm(self, assignment):
        ...
        log_br()
        log_info('MinimalExtendedAutomatonTask: building automaton...')
